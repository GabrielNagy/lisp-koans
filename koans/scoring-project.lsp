...
(defun score (dice)
  (let ((total-occurences (make-array '(6) :initial-element 0))
    ...
    (mapcar #'(lambda (die-roll)
            ...
            dice-indexes)
    (when (>= (aref total-occurences 0) 3) ; score triples of 1
      (setq score (+ score 1000))
      (setf (aref total-occurences 0) (- (aref total-occurences 0) 3)))
    (when (> (aref total-occurences 0) 0) ; score rest of 1
      (setq score (+ score (* (aref total-occurences 0) 100))))
    (when (> (aref total-occurences 4) 3) ; score non triples of 5
    ...
    (mapcar #'(lambda (die-index) ; score rest of triples
                (when (> die-index 0)
